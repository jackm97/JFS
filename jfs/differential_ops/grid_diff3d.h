...
public:
        ...
        static void Laplace(const gridBase* grid, SparseMatrix &dst, unsigned int dims, unsigned int fields=1);
        ...
        static void div(const gridBase* grid, SparseMatrix &dst, unsigned int fields=1);

        // calculate Gradient operator
        // Inputs:
        //      SparseMatrix &dst - destination sparse matrix for operator
        //      unsigned int fields - if there are multiple fields to concatenate (i.e. scalars concatenated by color channels)
        static void grad(const gridBase* grid, SparseMatrix &dst, unsigned int fields=1);
};
} // namespace jfs
